...
`;f.switchers=`	${i}::
  ...
  speed_${e}_timer:
    ...
    press_${e} = 0
    Return
`;_.getHover=(r,e,t,o,s)=>{return c(e)+c(o)+c(t)+c(s);function c(n){return`	${n}::
    If !move_${n} {
...
#If ${f.layer_condition.ext}
${_.getHover(f.extKey,Y,N,V,H)}
${_.getWheel(f.extKey,"q","wheelUp")}
${_.getWheel(f.extKey,"a","wheelDown")}#If`;function z(r=0){let e="";return a[r?"ext2Htk":"extHtk"].forEach((t,o)=>{switch(t){case"LButton":case"RButton":case"MButton":case"XButton1":case"XButton2":e+=`	${d[o]}::${t}
`;break;case"Capslock":e+=`	${d[o]}::SetCapsLockState, % GetKeyState("CapsLock","T") ? "Off" : "On"
		return
`;break;case"mouseL":H=d[o];break;case"mouseR":N=d[o];break;case"mouseU":Y=d[o];break;case"mouseD":V=d[o];break;case"brightnessUp":e+=K(d[o],1);break;case"brightnessDown":e+=K(d[o]);break;case"speed":e+=`	${d[o]}::
		speed_switcher := !speed_switcher
		resetSpeed()
		return
`;break;default:t!="."&&!t.includes("mouse")&&!t.includes("Wheel")&&(e+=`	${d[o]}::${t}
`)}}),e}function oe(){let r="";return a.sym.forEach((e,t)=>{+e||"0`\\/=[]".includes(e)?r+=`	${d[t]}::${e}
`:e>"~"?r+=`	${d[t]}::Send {${P(e)}}
		return
`:e!="."&&(r+=`	${d[t]}::SendRaw ${e}
		return
`)}),r}function ae(){let r="";return a.sym2.forEach((e,t)=>{e.startsWith("F")||e.startsWith("^")?r+=`	${d[t]}::${e}
`:e>"~"?r+=`	${d[t]}::Send {${P(e)}}
		return
`:e!="."&&(r+=`	${d[t]}::SendRaw ${e}
		return
`)}),r}function ce(){let r="";return a.symShift.forEach((e,t)=>{e=="%"?r+=`	${d[t]}::sendRaw \`%
		return
`:e>"~"?r+=`	${d[t]}::send {${P(e)}}
		return
`:e!="."&&(r+=`	${d[t]}::sendRaw ${e}
		return
`)}),r}function P(r){switch(r){case"\u26A0\uFE0F":return"U+26a0}{U+fe0f";default:return`U+${r.codePointAt(0).toString(16)}`}}function K(r,e=0){return e?`	${r}::ChangeBrightness(CurrentBrightness < 100 - brightnessJump ? CurrentBrightness += brightnessJump : 100)
    return
`:`	${r}::ChangeBrightness(CurrentBrightness > brightnessJump ? CurrentBrightness -= brightnessJump : 0)
    return
`}function ie(r){return new Promise(e=>setTimeout(e,r))}const J=document.createElement("ol");W.split(`
`).forEach((r,e)=>{const t=document.createElement("li");t.textContent=r,J.appendChild(t)});const y=document.createElement("button");y.id="copyBtn";y.textContent="copy";const b=document.createElement("span");b.textContent="copied \u2713";b.style.transition="opacity 500ms";y.prepend(b);y.addEventListener("click",async r=>{await navigator.clipboard.writeText(W),b.style.opacity=1,await ie(1e3),b.style.opacity=0},{once:!1});const C=document.createElement("div");C.id="ahkCtx";C.appendChild(y);C.appendChild(J);const B=document.createElement("div");B.id="kbdCtx";B.append(M,U);const G=document.createElement("main");G.append(B,C);q(G);document.body.append(G);R();se();console.log("\u{1F4A1}\u{1F4A1}done\u{1F4A1}\u{1F4A1}");
